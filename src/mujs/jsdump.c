...
const char *jsC_opcodestring(enum mjs_OpCode opcode)
{
...
static int prec(enum mjs_AstType type)
{
	switch (type) {
	case AST_IDENTIFIER:
	case EXP_IDENTIFIER:
...
static void pstmlist(int d, mjs_Ast *list);
static void pexpi(int d, int i, mjs_Ast *exp);
static void pstm(int d, mjs_Ast *stm);
static void slist(int d, mjs_Ast *list);
static void sblock(int d, mjs_Ast *list);

static void pargs(int d, mjs_Ast *list)
{
	while (list) {
...
static void parray(int d, mjs_Ast *list)
{
	pc('[');
	while (list) {
		assert(list->type == AST_LIST);
...
static void pobject(int d, mjs_Ast *list)
{
	pc('{');
	if (list) {
		nl();
...
static void pbin(int d, int p, mjs_Ast *exp, const char *op)
{
	pexpi(d, p, exp->a);
	sp();
	ps(op);
...
static void puna(int d, int p, mjs_Ast *exp, const char *pre, const char *suf)
{
	ps(pre);
	pexpi(d, p, exp->a);
	ps(suf);
...
static void pexpi(int d, int p, mjs_Ast *exp)
{
	int tp, paren;
...
static void pexp(int d, mjs_Ast *exp)
{
	pexpi(d, 0, exp);
}

static void pvar(int d, mjs_Ast *var)
{
...
static void pvarlist(int d, mjs_Ast *list)
{
	while (list) {
		assert(list->type == AST_LIST);
		pvar(d, list->a);
...
static void pblock(int d, mjs_Ast *block)
{
...
static void pstmh(int d, mjs_Ast *stm)
{
...
static void pcaselist(int d, mjs_Ast *list)
{
	while (list) {
		mjs_Ast *stm = list->a;
...
void jsP_dumpsyntax(mjs_State *J, mjs_Ast *prog, int dominify)
{
	minify = dominify;
...
static void snode(int d, mjs_Ast *node)
{
	void (*afun)(int,mjs_Ast*) = snode;
	void (*bfun)(int,mjs_Ast*) = snode;
	void (*cfun)(int,mjs_Ast*) = snode;
	void (*dfun)(int,mjs_Ast*) = snode;
...
void jsC_dumpfunction(mjs_State *J, mjs_Function *F)
{
	mjs_Instruction *p = F->code;
	mjs_Instruction *end = F->code + F->codelen;
	int i;
...
static void mjs_dumpproperty(mjs_State *J, mjs_Property *node)
{
	minify = 0;
	if (node->left->level)
		mjs_dumpproperty(J, node->left);
	printf("\t%s: ", node->name);
	mjs_dumpvalue(J, node->value);
	printf(",\n");
	if (node->right->level)
		mjs_dumpproperty(J, node->right);
}
...
void mjs_dumpobject(mjs_State *J, mjs_Object *obj)
{
	...
	if (obj->properties->level)
		mjs_dumpproperty(J, obj->properties);
	printf("}\n");
}
