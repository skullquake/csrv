...
static mjs_Ast *expression(mjs_State *J, int notin);
static mjs_Ast *assignment(mjs_State *J, int notin);
static mjs_Ast *memberexp(mjs_State *J);
static mjs_Ast *statement(mjs_State *J);
static mjs_Ast *funbody(mjs_State *J);

JS_NORETURN static void jsP_error(mjs_State *J, const char *fmt, ...) JS_PRINTFLIKE(2,3);
...
static void jsP_error(mjs_State *J, const char *fmt, ...)
{
	va_list ap;
	char buf[512];
	char msgbuf[256];
...
static void jsP_warning(mjs_State *J, const char *fmt, ...)
{
	va_list ap;
	char buf[512];
	char msg[256];
...
static mjs_Ast *jsP_newnode(mjs_State *J, enum mjs_AstType type, int line, mjs_Ast *a, mjs_Ast *b, mjs_Ast *c, mjs_Ast *d)
{
	mjs_Ast *node = mjs_malloc(J, sizeof *node);
...
static mjs_Ast *jsP_list(mjs_Ast *head)
{
	/* set parent pointers in list nodes */
	mjs_Ast *prev = head, *node = head->b;
	while (node) {
		node->parent = prev;
		prev = node;
		node = node->b;
...
static mjs_Ast *jsP_newstrnode(mjs_State *J, enum mjs_AstType type, const char *s)
{
	mjs_Ast *node = jsP_newnode(J, type, J->lexline, 0, 0, 0, 0);
	node->string = s;
	return node;
}

static mjs_Ast *jsP_newnumnode(mjs_State *J, enum mjs_AstType type, double n)
{
	mjs_Ast *node = jsP_newnode(J, type, J->lexline, 0, 0, 0, 0);
	node->number = n;
	return node;
...
static void jsP_freejumps(mjs_State *J, mjs_JumpList *node)
{
	while (node) {
		mjs_JumpList *next = node->next;
		mjs_free(J, node);
		node = next;
	}
}
...
{
	mjs_Ast *node = J->gcast;
	while (node) {
		mjs_Ast *next = node->gcnext;
...
static void jsP_next(mjs_State *J)
{
...
static void semicolon(mjs_State *J)
{
	if (J->lookahead == ';') {
		jsP_next(J);
		return;
...
static mjs_Ast *identifier(mjs_State *J)
{
	mjs_Ast *a;
...
static mjs_Ast *identifieropt(mjs_State *J)
{
	if (J->lookahead == TK_IDENTIFIER)
		return identifier(J);
	return NULL;
...
static mjs_Ast *identifiername(mjs_State *J)
{
	if (J->lookahead == TK_IDENTIFIER || J->lookahead >= TK_BREAK) {
		mjs_Ast *a = jsP_newstrnode(J, AST_IDENTIFIER, J->text);
		jsP_next(J);
		return a;
	}
...
static mjs_Ast *arrayelement(mjs_State *J)
{
	int line = J->lexline;
	if (J->lookahead == ',')
		return EXP0(UNDEF);
...
static mjs_Ast *arrayliteral(mjs_State *J)
{
	mjs_Ast *head, *tail;
	if (J->lookahead == ']')
		return NULL;
...
static mjs_Ast *propname(mjs_State *J)
{
	mjs_Ast *name;
...
static mjs_Ast *propassign(mjs_State *J)
{
	mjs_Ast *name, *value, *arg, *body;
	int line = J->lexline;
...
static mjs_Ast *objectliteral(mjs_State *J)
{
	mjs_Ast *head, *tail;
	if (J->lookahead == '}')
		return NULL;
...
static mjs_Ast *parameters(mjs_State *J)
{
	mjs_Ast *head, *tail;
	if (J->lookahead == ')')
		return NULL;
...
static mjs_Ast *fundec(mjs_State *J, int line)
{
	mjs_Ast *a, *b, *c;
	a = identifier(J);
	jsP_expect(J, '(');
...
static mjs_Ast *funstm(mjs_State *J, int line)
{
	mjs_Ast *a, *b, *c;
	a = identifier(J);
	jsP_expect(J, '(');
...
static mjs_Ast *funexp(mjs_State *J, int line)
{
	mjs_Ast *a, *b, *c;
	a = identifieropt(J);
	jsP_expect(J, '(');
...
static mjs_Ast *primary(mjs_State *J)
{
	mjs_Ast *a;
	int line = J->lexline;
...
static mjs_Ast *arguments(mjs_State *J)
{
	mjs_Ast *head, *tail;
	if (J->lookahead == ')')
		return NULL;
...
static mjs_Ast *newexp(mjs_State *J)
{
	mjs_Ast *a, *b;
	int line = J->lexline;
...
static mjs_Ast *callexp(mjs_State *J)
{
	mjs_Ast *a = newexp(J);
	int line;
	SAVEREC();
...
static mjs_Ast *postfix(mjs_State *J)
{
	mjs_Ast *a = callexp(J);
	int line = J->lexline;
...
static mjs_Ast *unary(mjs_State *J)
{
	mjs_Ast *a;
	int line = J->lexline;
	INCREC();
...
static mjs_Ast *multiplicative(mjs_State *J)
{
	mjs_Ast *a = unary(J);
	int line;
	SAVEREC();
...
static mjs_Ast *additive(mjs_State *J)
{
	mjs_Ast *a = multiplicative(J);
	int line;
	SAVEREC();
loop:
	INCREC();
...
static mjs_Ast *shift(mjs_State *J)
{
	mjs_Ast *a = additive(J);
	int line;
	SAVEREC();
loop:
	INCREC();
...
static mjs_Ast *relational(mjs_State *J, int notin)
{
	mjs_Ast *a = shift(J);
	int line;
	SAVEREC();
...
static mjs_Ast *equality(mjs_State *J, int notin)
{
	mjs_Ast *a = relational(J, notin);
	int line;
	SAVEREC();
loop:
	INCREC();
...
static mjs_Ast *bitand(mjs_State *J, int notin)
{
	mjs_Ast *a = equality(J, notin);
	SAVEREC();
	int line = J->lexline;
	while (jsP_accept(J, '&')) {
		INCREC();
...
static mjs_Ast *bitxor(mjs_State *J, int notin)
{
	mjs_Ast *a = bitand(J, notin);
	SAVEREC();
	int line = J->lexline;
	while (jsP_accept(J, '^')) {
		INCREC();
...
static mjs_Ast *bitor(mjs_State *J, int notin)
{
	mjs_Ast *a = bitxor(J, notin);
	SAVEREC();
	int line = J->lexline;
	while (jsP_accept(J, '|')) {
		INCREC();
...
static mjs_Ast *logand(mjs_State *J, int notin)
{
	mjs_Ast *a = bitor(J, notin);
	int line = J->lexline;
...
static mjs_Ast *logor(mjs_State *J, int notin)
{
	mjs_Ast *a = logand(J, notin);
	int line = J->lexline;
...
static mjs_Ast *conditional(mjs_State *J, int notin)
{
	mjs_Ast *a = logor(J, notin);
	int line = J->lexline;
	if (jsP_accept(J, '?')) {
		mjs_Ast *b, *c;
		INCREC();
...
static mjs_Ast *vardec(mjs_State *J, int notin)
{
	mjs_Ast *a = identifier(J);
	int line = J->lexline;
	if (jsP_accept(J, '='))
...
static mjs_Ast *vardeclist(mjs_State *J, int notin)
{
	mjs_Ast *head, *tail;
...
static mjs_Ast *statementlist(mjs_State *J)
{
	mjs_Ast *head, *tail;
...
static mjs_Ast *caseclause(mjs_State *J)
{
	mjs_Ast *a, *b;
	int line = J->lexline;
...
static mjs_Ast *caselist(mjs_State *J)
{
	mjs_Ast *head, *tail;
	if (J->lookahead == '}')
		return NULL;
...
static mjs_Ast *block(mjs_State *J)
{
	mjs_Ast *a;
	int line = J->lexline;
	jsP_expect(J, '{');
...
static mjs_Ast *forexpression(mjs_State *J, int end)
{
	mjs_Ast *a = NULL;
	if (J->lookahead != end)
		a = expression(J, 0);
...
static mjs_Ast *forstatement(mjs_State *J, int line)
{
	mjs_Ast *a, *b, *c, *d;
	jsP_expect(J, '(');
...
static mjs_Ast *scriptelement(mjs_State *J)
{
	int line = J->lexline;
	if (jsP_accept(J, TK_FUNCTION))
...
static mjs_Ast *script(mjs_State *J, int terminator)
{
	mjs_Ast *head, *tail;
...
static int jsP_setnumnode(mjs_Ast *node, double x)
{
...
static int jsP_foldconst(mjs_Ast *node)
{
	double x, y;
	int a, b;
