...
JS_NORETURN static void jsY_error(mjs_State *J, const char *fmt, ...) JS_PRINTFLIKE(2,3);

static void jsY_error(mjs_State *J, const char *fmt, ...)
{
	va_list ap;
	char buf[512];
	char msgbuf[256];
...
static int jsY_findkeyword(mjs_State *J, const char *s)
{
...
static void jsY_next(mjs_State *J)
{
	Rune c;
...
static void jsY_unescape(mjs_State *J)
{
	if (jsY_accept(J, '\\')) {
		if (jsY_accept(J, 'u')) {
			int x = 0;
...
static void textinit(mjs_State *J)
{
	if (!J->lexbuf.text) {
		J->lexbuf.cap = 4096;
		J->lexbuf.text = mjs_malloc(J, J->lexbuf.cap);
	}
	J->lexbuf.len = 0;
}

static void textpush(mjs_State *J, Rune c)
{
	int n = runelen(c);
	if (J->lexbuf.len + n > J->lexbuf.cap) {
		J->lexbuf.cap = J->lexbuf.cap * 2;
		J->lexbuf.text = mjs_realloc(J, J->lexbuf.text, J->lexbuf.cap);
	}
	J->lexbuf.len += runetochar(J->lexbuf.text + J->lexbuf.len, &c);
}

static char *textend(mjs_State *J)
{
	textpush(J, 0);
	return J->lexbuf.text;
}

static void lexlinecomment(mjs_State *J)
{
	while (J->lexchar && J->lexchar != '\n')
		jsY_next(J);
}

static int lexcomment(mjs_State *J)
{
...
static double lexhex(mjs_State *J)
{
	double n = 0;
...
static double lexinteger(mjs_State *J)
{
	double n = 0;
...
static double lexfraction(mjs_State *J)
{
	double n = 0;
	double d = 1;
	while (jsY_isdec(J->lexchar)) {
...
static double lexexponent(mjs_State *J)
{
	double sign;
...
static int lexnumber(mjs_State *J)
{
	double n;
	double e;
...
static int lexnumber(mjs_State *J)
{
	...
	J->number = mjs_strtod(s, NULL);
	return TK_NUMBER;
}

#endif

static int lexescape(mjs_State *J)
{
	int x = 0;
...
static int lexstring(mjs_State *J)
{
	const char *s;
...
static int lexregexp(mjs_State *J)
{
	const char *s;
	int g, m, i;
	int inclass = 0;
...
static int jsY_lexx(mjs_State *J)
{
	J->newline = 0;
...
void jsY_initlex(mjs_State *J, const char *filename, const char *source)
{
	J->filename = filename;
	J->source = source;
	J->line = 1;
...
int jsY_lex(mjs_State *J)
{
	return J->lasttoken = jsY_lexx(J);
}

static int lexjsonnumber(mjs_State *J)
{
	const char *s = J->source - 1;
...
static int lexjsonescape(mjs_State *J)
{
	int x = 0;
...
static int lexjsonstring(mjs_State *J)
{
	const char *s;
