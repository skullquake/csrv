...
function pad(pad, str, padLeft) {
  ...
  if (padLeft) {
	...
	var style=$("<style/>").text(`
		...
		.${cssclass} #graph>hr.hline{
			position:absolute;
			width:100%;
			left:0px;
			margin:0px;
		}
		.${cssclass} #graph>hr.hline#avg{
			color:#FFFFFF;
		}
		...
		.${cssclass} *{
			font-family:ProggySquareTTSZ!important;;
	...
	var div_graph=$("<div/>").attr({"id":"graph"});
		var lbl_graph_min=$("<span/>").addClass("lbl").attr({"id":"lbl_min"});
		var lbl_graph_max=$("<span/>").addClass("lbl").attr({"id":"lbl_max"});
		var hr_graph_avg=$("<hr/>").addClass("hline").attr({"id":"avg"});
		div_graph.append(lbl_graph_max)
		div_graph.append(lbl_graph_min)
		div_graph.append(hr_graph_avg);
	...
	function updateGraph(){
		if(graphdivbuf.length!=graphdata.length){
			div_graph.find("div").remove();
			graphdivbuf=[];
			for(var i=0;i<graphdata.length;i++){
				...
				div_graph.append(div);
				graphdivbuf.push(div);
			}
		}
		var maxval=0;
		var avg=0;
		...
		lbl_graph_min.text(0);
		lbl_graph_max.text(maxval);
		graphdata.forEach(function(val,validx){
			val=val/maxval*100;
			avg+=val;
			graphdivbuf[validx].css({
				"height":`${val}%`,
				"background":`hsl(${val},100%,50%)`,
			}).text("");
		/*
		...
		*/
		...
		avg/=graphdata.length;
		hr_graph_avg.css({"bottom":avg+"%"});
	};
